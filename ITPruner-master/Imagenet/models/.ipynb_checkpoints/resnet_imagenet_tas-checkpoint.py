...
class Bottleneck(nn.Module):
    def __init__(self, inplanes, planes_1, planes_2, planes_3=0, stride=1, downsample=None, norm_layer=None):
        ...
        if stride == 2:
            self.downsample = nn.Sequential(OrderedDict([('avg',nn.AvgPool2d(kernel_size=2, stride=2, padding=0)),('conv', conv1x1(inplanes, planes_3)),
                                                    ('bn', norm_layer(planes_3))]))
        elif inplanes != planes_3:
            self.downsample = nn.Sequential(OrderedDict([('conv', conv1x1(inplanes, planes_3)),
                                                    ('bn', norm_layer(planes_3))]))
        else:
...
class ResNet_ImageNet_TAS(MyNetwork):
    def __init__(self, cfg=None, depth=18, block=BasicBlock, num_classes=1000):
        super(ResNet_ImageNet_TAS, self).__init__()
        self.cfgs_base = {18: [64, 64, 64, 64, 128, 128, 128, 256, 256, 256, 512, 512, 512],
                          ...
                          50: [3, 45, 45, 30, 102, 33, 60, 154, 68, 70, 180, 38, 38, 307, 38, 38, 410, 64, 128, 358, 38, 51, 256, 76, 76, 512, 76, 76, 512, 179, 256, 614, 100, 102, 307, 179, 230, 614, 204, 102, 307, 153, 153, 1228, 512, 512, 1434, 512, 512, 1844]}
        if depth==18:
            block = BasicBlock
            blocks = [2, 2, 2, 2]
        ...
        self.inplanes = cfg[1]
        self.blocks = blocks
        self.conv1 = nn.Sequential(OrderedDict([('conv', nn.Conv2d(cfg[0], self.inplanes, kernel_size=7, stride=2, padding=3, bias=False)),
        ...
        if depth!=50:
            ...
            self.layer1 = self._make_layer(block, cfg[idx+1: 3*blocks[0]+idx+1], blocks[0])
            self.layer2 = self._make_layer(block, cfg[3*blocks[0]+idx+1: 3*blocks[0]+3*blocks[1]+idx+1], blocks[1], stride=2)
            self.layer3 = self._make_layer(block, cfg[3*blocks[0]+3*blocks[1]+idx+1:3*blocks[0]+3*blocks[1]+3*blocks[2]+idx+1], blocks[2], stride=2)
            self.layer4 = self._make_layer(block, cfg[3*blocks[0]+3*blocks[1]+3*blocks[2]+idx+1: ], blocks[3], stride=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        if self.depth == 50:
            ...
            for i in range(blocks):
                first_planes = planes[i*3:(i+1)*3]
                if i == 0:
                    layers.append(block(self.inplanes, first_planes[0], first_planes[1], first_planes[2], stride, norm_layer))
                else:
                    layers.append(block(self.inplanes, first_planes[0], first_planes[1], first_planes[2], 1, norm_layer))
                self.inplanes = first_planes[2]
                if blocks==6 and i==4:
                    break
    ...
    def cfg2flops(self, cfg):  # to simplify, only count convolution flops
        ...
        flops += (cfg[0] * 7 * 7 * cfg[1] * size * size + 5 * cfg[1] * size * size) # first layer, conv+bn+relu
        inplanes = cfg[1]
        size /= 2 # pooling s=2
        flops += (3 * 3 * cfg[1] * size * size) # maxpooling
        if self.depth != 50:
            sub_cfgs = [cfg[1 : blocks[0]+2],
                        cfg[blocks[0]+2 : blocks[0]+2+blocks[1]+1], 
                        cfg[blocks[0]+blocks[1]+3 : blocks[0]+blocks[1]+blocks[2]+4],
            ...
            sub_cfgs = [cfg[1+1: 3*blocks[0]+1+1],
                        cfg[3*blocks[0]+1+1: 3*blocks[0]+3*blocks[1]+1+1],
                        cfg[3*blocks[0]+3*blocks[1]+1+1:3*blocks[0]+3*blocks[1]+3*blocks[2]+1+1],
                        cfg[3*blocks[0]+3*blocks[1]+3*blocks[2]+1+1: ]]
        for i in range(4): # each layer
            ...
            if self.depth != 50:
                ...
                first_planes = planes[0:3]
                later_planes = planes[3:3+3*(blocks[i]-1)]
            if i in [1, 2, 3]:
                size /= 2
            ...
            for j in range(1, self.blocks[i]):
                ...
                if self.depth != 50:
                    ...
                    flops += (inplanes * 1 * 1 * later_planes[3*(j-1)] + 5 * later_planes[3*(j-1)]) * size * size
                    flops += (later_planes[3*(j-1)] * 3 * 3 * later_planes[3*(j-1)+1] + 5 * later_planes[3*(j-1)+1]) * size * size
                    flops += (later_planes[3*(j-1)+1] * 1 * 1 * later_planes[3*(j-1)+2] + 5 * later_planes[3*(j-1)+2]) * size * size
